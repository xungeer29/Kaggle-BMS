...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, vocab_size, device, encoder_dim=512, dropout=0.5, rnn='lstm'):
        """
        :param attention_dim: input size of attention network
        :param embed_dim: input size of embedding network
        :param decoder_dim: input size of decoder network
        :param vocab_size: total number of characters used in training
        :param encoder_dim: input size of encoder network channel
    ...
    def init_hidden_state(self, encoder_out):
        ...
        h = self.init_h(mean_encoder_out)  # (batch_size, h*w, decoder_dim)
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        encoder_out = encoder_out.view(batch_size, -1, encoder_dim)  # (batch_size, num_pixels, encoder_dim) num_pixels: h*w
        ...
        for t in range(max(decode_lengths)):
            ...
            if self.rnn == 'lstm':
                h, c = self.decode_step(
                    ...
                    (h[:batch_size_t], c[:batch_size_t]))  # (batch_size_t, decoder_dim) LSTM
            elif self.rnn == 'gru':
                h = self.decode_step(
                    torch.cat([embeddings[:batch_size_t, t, :], attention_weighted_encoding], dim=1),
                    h[:batch_size_t])  # (batch_size_t, decoder_dim) GRU
        ...
        for t in range(decode_lengths):
            ...
            if self.rnn == 'lstm':
                h, c = self.decode_step(
                    ...
                    (h, c))  # (batch_size_t, decoder_dim) LSTM
            elif self.rnn == 'gru':
                h = self.decode_step(
                    torch.cat([embeddings, attention_weighted_encoding], dim=1), h)  # (batch_size_t, decoder_dim) GRU
            preds = self.fc(self.dropout(h))  # (batch_size_t, vocab_size)
            predictions[:, t, :] = preds
