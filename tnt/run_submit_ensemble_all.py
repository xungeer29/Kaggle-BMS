...
def fast_remote_unrotate_augment(r):
    ...
    # original entire image   denoise
    img2 = cv2.resize(img_denoise, dsize=(224, 224), interpolation=cv2.INTER_LINEAR)
    img2 = img2.astype(np.float16) / 255
    ...
    # crop2rect denoise
    image2 = crop_image_to_rect(img_denoise)
    image2 = cv2.resize(image2, dsize=(224, 224), interpolation=cv2.INTER_LINEAR)
    image2 = image2.astype(np.float16) / 255
    image2 = torch.from_numpy(image2).unsqueeze(0).repeat(3,1,1)
    ...
    img_lstm_denoise = augs(image=img_lstm_denoise)['image']

    # lstm eb0 224
    img_eb0 = np.stack((image, image, image), axis=2)
    img_eb0_rot = np.stack((img_rot90, img_rot90, img_rot90), axis=2)
    img_eb0_denoise = np.stack((img_denoise, img_denoise, img_denoise), axis=2)
    augs = A.Compose([
            A.Resize(224, 224),
            A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225],),
            ToTensorV2(),])
    img_eb0 = augs(image=img_eb0)['image']
    img_eb0_rot = augs(image=img_eb0_rot)['image']
    img_eb0_denoise = augs(image=img_eb0_denoise)['image']

    r={}
    r['img_crop'] = img_crop # orig
    r['img_crop_rot'] = image1
    r['img_crop_denoise'] = image2
    ...
    r['img_lstm_denoise'] = img_lstm_denoise

    r['img_eb0'] = img_eb0
    r['img_eb0_rot'] = img_eb0_rot
    r['img_eb0_denoise'] = img_eb0_denoise
...
def do_predict(nets, tokenizer, valid_loader, ckpts):
    ...
    for t, batch in enumerate(valid_loader):
        ...
        img_crop_denoise = batch['img_crop_denoise'].cuda() # entire
        img_crop = torch.cat((img_crop, img_crop_rot, img_crop_denoise), dim=0)

        img = batch['img'].cuda() # entire
        img_rot = batch['img_rot'].cuda()
        img_denoise = batch['img_denoise'].cuda()
        img = torch.cat((img, img_rot, img_denoise), dim=0)

        batch_size = img_crop.shape[0]
        # lstm
        img_lstm = batch['img_lstm'].cuda()
        img_lstm_rot = batch['img_lstm_rot'].cuda()
        img_lstm_denoise = batch['img_lstm_denoise'].cuda()
        img_lstm = torch.cat((img_lstm, img_lstm_rot, img_lstm_denoise), dim=0)

        # eb0
        img_eb0 = batch['img_eb0'].cuda()
        img_eb0_rot = batch['img_eb0_rot'].cuda()
        img_eb0_denoise = batch['img_eb0_denoise'].cuda()
        img_eb0 = torch.cat((img_eb0, img_eb0_rot, img_eb0_denoise), dim=0)
        ...
        with torch.no_grad():
            with amp.autocast():
                ...
                for i, net in enumerate(nets[:(len(ckpts[0]+ckpts[1]))]):
                ...
                for i, net in enumerate(nets[(len(ckpts[0]+ckpts[1])):]):
                    encoder_out = net['encoder'](img_lstm) if i < len(ckpts[2]) else net['encoder'](img_eb0)
                ...
                for t in range(max_length-1):
                    ...
                    x = (x[:real_bs] + x[real_bs:2*real_bs] + x[2*real_bs:3*real_bs]) / (batch_size/real_bs)
...
def run_submit():
    ...
    ckpts04 = [
    '../logs/v12/fold0_best.pth',
    ]
    ckpts = [ckpts00, ckpts01, ckpts02, ckpts03, ckpts04]
    is_norm_ichi = False #True
    # out_dir = 'logs/ensemble/entire-crop-lstm'
    out_dir = 'logs/ensemble/tnt(crop-entire)-lstm(eb4-eb0)'
    ...
    for ckpt in ckpts03:
        ...
        encoder = Encoder('tf_efficientnet_b4_ns', pretrained=False)
    ...
    from lstm.v12 import Encoder, Attention, DecoderWithAttention
    for ckpt in ckpts04:
        net = {}
        states = torch.load(ckpt, map_location=torch.device('cpu'))
        encoder = Encoder('efficientnet_b0', pretrained=False)
        encoder.load_state_dict(states['encoder'])
        encoder.cuda()
        encoder.eval()
    ...
    df_submit.to_csv(out_dir + '/submit_3.csv', index=False)
    print(out_dir + '/submit_3.csv')
