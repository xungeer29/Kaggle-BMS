...
class TransformerDecode(FairseqIncrementalDecoder):
    def __init__(self, dim, ff_dim, num_head, num_layer):
        ...
        # self.rew = torch.nn.Parameter(torch.tensor(0.), requires_grad=True) # luozhengbo

    def forward(self, x, mem, x_mask):
            # x_ = x # luozhengbo
            for layer in self.layer:
                x = layer(x, mem, self_attn_mask=x_mask)[0]
            # x = x_ + self.rew * x # luozhengbo
            x = self.layer_norm(x)
            return x  # T x B x C
...
class TransformerDecode_rew(FairseqIncrementalDecoder):
    def __init__(self, dim, ff_dim, num_head, num_layer):
        super().__init__({})
        #print('my TransformerDecode()')
        ...
        self.rew = torch.nn.Parameter(torch.tensor(0.), requires_grad=True) # luozhengbo re-weight
