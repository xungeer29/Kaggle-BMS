...
default_cfgs = {
    ...
    'tnt_s_patch16_320': _cfg(
        mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5),
        input_size=(3, 320, 320),
    ),
    'tnt_b_patch16_320': _cfg(
        mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5),
        input_size=(3, 320, 320),
    ),
}
...
@register_model
def tnt_s_patch16_320(pretrained=False, **kwargs):
    model = TNT(img_size=320, patch_size=16, embed_dim=384, in_dim=24, depth=12, num_heads=6, in_num_head=4,
                qkv_bias=False, **kwargs)
    model.default_cfg = default_cfgs['tnt_s_patch16_320']
    if pretrained:
        load_pretrained(
            model, num_classes=model.num_classes, in_chans=kwargs.get('in_chans', 3))
    return model
...
@register_model
def tnt_b_patch16_320(pretrained=False, **kwargs):
    model = TNT(img_size=320, patch_size=16, embed_dim=640, in_dim=40, depth=12, num_heads=10, in_num_head=4,
                qkv_bias=False, **kwargs)
    model.default_cfg = default_cfgs['tnt_b_patch16_320']
    if pretrained:
        load_pretrained(
            model, num_classes=model.num_classes, in_chans=kwargs.get('in_chans', 3))
    return model